...
class oehealth_group_category(osv.osv):
    _name = 'oehealth.group.category'
    _description = 'Group Categories'

    def name_get(self, cr, uid, ids, context=None):
        """Return the category's display name, including their direct
           parent by default.

        :param dict context: the ``group_category_display`` key can be
        ...
        if context is None:
            context = {}
        if context.get('group_category_display') == 'short':
            return super(oehealth_group_category, self).name_get(cr, uid, ids, context=context)
    ...
    _columns = {
        'name': fields.char('Group Category', required=True, size=64, translate=True),
        'parent_id': fields.many2one('oehealth.group.category', 'Parent Category', select=True, ondelete='cascade'),
        'description': fields.text(string='Description'),
        'info': fields.text(string='Info'),
        'complete_name': fields.function(_name_get_fnc, type="char", string='Group Category'),
        'child_ids': fields.one2many('oehealth.group.category', 'parent_id', 'Child Categories'),
        ...
        'group_ids': fields.many2many('oehealth.group', 

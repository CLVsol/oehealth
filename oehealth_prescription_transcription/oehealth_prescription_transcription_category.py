...
class oehealth_prescription_trasncription_category(osv.osv):
    _name = 'oehealth.prescription.transcription.category'
    _description = 'Healh Prescription Transcription Categories'

    def name_get(self, cr, uid, ids, context=None):
        """Return the category's display name, including their direct
           parent by default.

        :param dict context: the ``prescription_transcriptiption_category_display`` key can be
        ...
        if context is None:
            context = {}
        if context.get('prescription_transcriptiption_category_display') == 'short':
            return super(oehealth_prescription_trasncription_category, self).name_get(cr, uid, ids, context=context)
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('oehealth.prescription.transcription.category', 'Parent Category', select=True, ondelete='cascade'),
        'description': fields.text(string='Description'),
        'info': fields.text(string='Info'),
        'complete_name': fields.function(_name_get_fnc, type="char", string='Category'),
        'child_ids': fields.one2many('oehealth.prescription.transcription.category', 'parent_id', 'Child Categories'),
...
oehealth_prescription_trasncription_category()

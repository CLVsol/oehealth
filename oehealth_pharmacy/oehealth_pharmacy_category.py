...
class oehealth_pharmacy_category(osv.osv):
    _name = 'oehealth.pharmacy.category'
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        if context.get('pharmacy_category_display') == 'short':
            return super(oehealth_pharmacy_category, self).name_get(cr, uid, ids, context=context)
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('oehealth.pharmacy.category', 'Parent Category', select=True, ondelete='cascade'),
        'description': fields.text(string='Description'),
        'info': fields.text(string='Info'),
        'complete_name': fields.function(_name_get_fnc, type="char", string='Category'),
        'child_ids': fields.one2many('oehealth.pharmacy.category', 'parent_id', 'Child Categories'),
        'active': fields.boolean('Active', help="The active field allows you to hide the category without removing it."),
        'parent_left': fields.integer('Left parent', select=True),
        'parent_right': fields.integer('Right parent', select=True),
        'pharmacy_ids': fields.many2many('oehealth.pharmacy', 

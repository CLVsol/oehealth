...
class oehealth_specialty(osv.Model):
    _name = 'oehealth.specialty'
    _description = 'Health Specialty'

    def name_get(self, cr, uid, ids, context=None):
        """Return the specialty' display name, including their direct
           parent by default.

        :param dict context: the ``oehealth_specialty_display`` key can be
        ...
        if context is None:
            context = {}
        if context.get('oehealth_specialty_display') == 'short':
            return super(oehealth_specialty, self).name_get(cr, uid, ids, context=context)
    ...
    _columns = {
        'name': fields.char(size=256, string='Health Specialty', required=True, translate=True),
        ...
        'parent_id': fields.many2one('oehealth.specialty', 'Parent Specialty', select=True, ondelete='cascade'),
        'complete_name': fields.function(_name_get_fnc, type="char", string='Health Specialty'),
        'child_ids': fields.one2many('oehealth.specialty', 'parent_id', 'Child Specialties'),
        'parent_left': fields.integer('Left parent', select=True),
        'parent_right': fields.integer('Right parent', select=True),
        'tag_ids': fields.many2many('oehealth.tag', 
                                    'oehealth_specialty_tag_rel', 
                                    'specialty_id', 
                                    'tag_id', 
                                    'Tags'),
